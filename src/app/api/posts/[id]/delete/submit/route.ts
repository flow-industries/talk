...
export async function POST(req: NextRequest) {
  try {
    const body: SubmitDeleteRequest = await req.json();
    const { signTypedDataParams, appSignature, authorSignature, deleteData, chainId } = body;

    if (!signTypedDataParams || !appSignature || !authorSignature || !deleteData) {
    ...
    const isAuthorSignatureValid = await submitterWalletClient.verifyTypedData({
      ...
      address: signTypedDataParams.message.author as `0x${string}`,
    });
    ...
    const isAppSignatureValid = await submitterWalletClient.verifyTypedData({
      ...
      address: deleteData.app as `0x${string}`,
    });
    ...
    const { txHash } = await deleteCommentWithSig({
      commentId: deleteData.commentId as `0x${string}`,
      app: deleteData.app as `0x${string}`,
      deadline: BigInt(deleteData.deadline),
