...
export async function POST(req: NextRequest) {
  try {
    const body: SubmitEditRequest = await req.json();
    const { signTypedDataParams, appSignature, authorSignature, editData, chainId } = body;

    if (!signTypedDataParams || !appSignature || !authorSignature || !editData) {
    ...
    const isAppSignatureValid = await submitterWalletClient.verifyTypedData({
      ...
      address: editData.app as `0x${string}`,
    });
    ...
    const { txHash } = await editCommentWithSig({
      edit: signTypedDataParams.message,
