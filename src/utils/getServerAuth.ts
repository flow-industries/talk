...
// Lightweight auth check - only reads session, no ENS user fetch
// Use this for API routes that only need to know the user's address
...
// Using React's cache() to deduplicate calls within a single request
// This ensures getServerAuth() only executes once per request even if called multiple times
